...
namespace kmeal {
    uint64_t kmeal::get_resource( uint8_t resource ) {
    ...
    uint64_t kmeal::get_cap( uint8_t resource, asset quantity ) {
    ...
    void kmeal::staking( account_name from, account_name to, uint8_t resource, asset quantity ) {
        require_auth( from );
    ...
    void kmeal::addstake( account_name from, account_name to, uint8_t resource, uint64_t staking, asset balance ) {
    ...
    void kmeal::addamount( account_name account, uint8_t resource, uint64_t staking, asset balance ) {
    ...
    void kmeal::add_total_staking( uint8_t resource, uint64_t staking ) {
        totalIndex totalresource( _self, resource );
        auto itr = totalresource.find( resource );
